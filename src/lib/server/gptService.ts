...
import { Message } from "../../types";
  ...
  async getQuestion(query: string, userContext: UserContext): Promise<Question> {
    ...
    } else if (userContext.age < 16) {
      difficultyLevel = "intermediate";
    } else if (userContext.age < 20) {
      difficultyLevel = "advanced";
    } else {
      difficultyLevel = "expert";
    }

    const systemPrompt = `You are an expert tutor. Generate a ${difficultyLevel}-level multiple choice question about ${query}. 
      The question should test deep understanding and critical thinking, not just memorization.
      The question should be formatted in markdown.
      Include 4 options with only one correct answer.
      Provide a detailed explanation of why the correct answer is right and why others are wrong.
      The explanation should be educational and help the student learn from their mistakes.
      
      Adjust the complexity and depth based on the student's age (${userContext.age} years old).
      ...
      Return the response in this exact JSON format:
      {
        ...
        "explanation": {
          ...
          "incorrect": ["explanation for incorrect option 1", "explanation for incorrect option 2", "explanation for incorrect option 3"]
        },
        "difficulty": ${userContext.age < 12 ? 1 : userContext.age < 16 ? 2 : userContext.age < 20 ? 3 : 4},
        "topic": "${query}",
        "subtopic": "relevant subtopic"
      }`;

    const response = await this.makeRequest(systemPrompt, query);
  ...
  private validateQuestionFormat(question: Question): boolean {
    try {
      ...
      if (!question.explanation?.correct?.trim() || 
          !question.explanation?.incorrect?.every(opt => opt?.trim())) return false;
      ...
      if (question.explanation.correct.length < 5 || 
          question.explanation.incorrect.some(opt => opt.length < 5)) return false; // Too short explanations
    ...
    try {
      ...
      const formattedQuestion: Question = {
        ...
        explanation: {
          ...
          incorrect: shuffled.explanation?.incorrect || ['Key learning point']
        },
        difficulty: level,
        topic: topic,
  ...
  async streamExploreContent(
    ...
    while (retryCount < maxRetries) {
      try {
        ...
        let mainContent = '';
        let jsonContent = '';
        const currentTopics: Array<{
          topic: string;
          type: string;
          reason: string;
        }> = [];
        const currentQuestions: Array<{
          question: string;
          type: string;
          context: string;
        }> = [];
              ...
              if (jsonContent.includes('}')) {
                ...
                if (jsonStr.startsWith('{') && jsonStr.endsWith('}')) {
                  ...
                  if (parsed.topics && Array.isArray(parsed.topics)) {
                    parsed.topics.forEach((topic: ParsedTopic) => {
                  ...
                  if (parsed.questions && Array.isArray(parsed.questions)) {
                    parsed.questions.forEach((question: ParsedQuestion) => {
